...
context('Slot widgets grid', () => {
  it('displays slot widget correctly', () => {
    cy.intercept('GET', '**/slot-widgets', {
      fixture: 'slot-widgets/success.json',
    }).as('getSlotWidgets');
    cy.intercept('GET', '**/fares/**', { fixture: 'fares/success.json' }).as(
      'getFare'
    );
    cy.visit('/');
  ...
  it('displays an error if fare not loaded', function () {
    ...
    cy.findByText('Details for this widget are temporary unavailable.', {
      timeout: 1000,
    }).should('exist');
  });
  ...
  it('displays an error if slot widgets are failed to load', () => {
    ...
    cy.findByText("Error Network Error occurred. We'll get back to you soon!", {
      timeout: 1000,
    }).should('exist');
  });
});
